#!/usr/bin/env anduril
//$OPT --threads 14
//$OPT -d /mnt/storage2/work/amjad/ctdna/result_newMutectAllv2
...
object ctdna{
  ...
  val list2 = INPUT(path = "/mnt/storage2/work/amjad/ctdna/result_ctdnaAlignmentAllv2/bamCorrectedOutCSV/out.csv")
...
for ( rowMap <- iterCSV(outBamArrayCSVmerged) ) { 
  ...
  varsFiltered(patient) = BashEvaluate(var1 = vars(patient).out1,
     ...
     script = s"$java8 -jar $gatk FilterMutectCalls -R @var3@ -V @var1@  -O @out1@ --stats @var1@.stats --filtering-stats @out2@ -ob-priors @var2@ " +
...
*/
 ...
 if(patient != "CHIC_51" & patient != "CHIC_46"){

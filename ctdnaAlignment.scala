...
object ctdna{
  ...
  val list = INPUT(path = "/mnt/storage1/rawdata/ctDNA/list.csv")
  ...
  for ( rowMap <- iterCSV(list) ) {
    ...
    reads(Sample) = INPUT(path = rowMap("reads"))
    mates(Sample) = INPUT(path = rowMap("mates"))
    umi(Sample)   = INPUT(path = rowMap("UMI"))
    

    // alignment using bwa-mem
    aligned(Sample) = BashEvaluate(var1 = reads(Sample),
        var2 = mates(Sample),
        var3 = reference,
        param1 = Sample,
        script = s"""
                 ...
                 """)
    aligned(Sample)._keep = false
    aligned(Sample)._filename("out1","aligned.bam")
    aligned(Sample)._execute = "once"

    // attach Umis to aligned bam file from a fastq
    alignedUmi(Sample) = BashEvaluate(var1 = aligned(Sample).out1,
        var2 = umi(Sample),
        script = s"""
                  java -jar $fgbio AnnotateBamWithUmis -i @var1@ -f @var2@ -o @out1@ 
                  """)
    alignedUmi(Sample)._keep = false
    alignedUmi(Sample)._filename("out1","alignedUmi.bam")
    alignedUmi(Sample)._execute = "once"

    // sort reads by coordinates
    sorted(Sample) = BashEvaluate(var1 = alignedUmi(Sample).out1,
        var2   = reference,
        param1 = picard,
        script = """java -Xmx4g -jar @param1@ SortSam VALIDATION_STRINGENCY=SILENT SORT_ORDER=coordinate \
               ...
               """)
    sorted(Sample)._filename("out1","sorted.bam")
    sorted(Sample)._execute = "once"
    
    // markd duplicates (UMI aware)
    marked(Sample) = BashEvaluate(var1 = sorted(Sample).out1,
    	param1 = picard,
        script = """
   ...
   // extract markDuplicates metrics
    ...
    groupedUmi(Sample) = BashEvaluate(var1 = sorted(Sample).out1,
       var2 = reference,
       param1 = fgbio,
       script = s"""
   ...
   // sort reads by TemplateCoordinate
    sortedTemplates(Sample) = BashEvaluate(var1 = marked(Sample).out1,
        param1 = fgbio,
        script = s"""
                 java -jar $fgbio SortBam -s TemplateCoordinate -i @var1@ -o @out1@
                 """)
    sortedTemplates(Sample)._filename("out1","out1.bam")
    ...
    consensusedFilter(Sample) = BashEvaluate(var1 = consensused(Sample).out1,
        ...
        script = """
                 java -jar @param1@ FilterConsensusReads \
                   -i @var1@ -o @out1@ -r @var2@ -M 1 -N 10 -q 15
                 """)
    ...
    consensusSorted(Sample) = BashEvaluate(var1 = consensusAligned(Sample).out1,
        ...
        script = """java -Xmx4g -jar @param1@ SortSam VALIDATION_STRINGENCY=SILENT SORT_ORDER=coordinate \
               CREATE_INDEX=true CREATE_MD5_FILE=true INPUT=@var1@ OUTPUT=@out1@ TMP_DIR=$( gettempdir )
               """)
    sorted(Sample)._filename("out1",Sample + "_CorrectedSorted.bam")
...
/*
 val sortedCSV = Array2CSV(in = sortedOut)
 val sortedArray = CSV2Array(in = sortedCSV,
   keys = "column")
 
 val outBam = NamedMap[Any]("outBam")
 val statSummary = NamedMap[Any]("statSummary")
 val complexity = NamedMap[Any]("complexity")
  
  for ((k,v) <- iterArray(sortedArray.out))
  {
    withName(k) 
    {   
		
    
    val marked = DuplicateMarker(in=sortedArray.out(k),
                                 picard="/opt/share/picard-2.18.13/")
    ...
    val recal = BaseRecalibrator(reference = reference,
                                 ...
                                 dbsnp = dbsnp,
                                 mask = indelsGold,
                                 gatk = "/opt/share/gatk-3.7/",
                                 plot= false)
     
    val stats = BamStats(bam = recal.alignment,
                         refGenome = reference,
                         targets = targetsPadded,
                         markStats = marked.outMetrics,
                         targeted=true,
                         sampleName=k,
                         maxCoverage=500,
                         picard="/opt/share/picard-2.18.13/",
                         bedtools="/opt/share/bedtools-2.17.0/bin/")                                 	 
    
    
  
    val libraryComplexity = BashEvaluate(var1 = recal.alignment,
             script = """
                 java -jar /opt/share/picard-2.18.13/picard.jar EstimateLibraryComplexity \
                    I=@var1@  O=@out1@    
                 head -n 8 @out1@ | tail -n 2 > @out2@   
                      """)   
                                     
    statSummary(k) = stats.summary
    
    outBam(k) = recal.alignment  
    complexity(k) = libraryComplexity.out2

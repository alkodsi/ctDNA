#!/usr/bin/env anduril
//$OPT --threads 15
//$OPT -d /mnt/storage2/work/amjad/ctdna/result_ctDNAlign
...
object ctdna{
  ...
  val list1 = INPUT(path = "/mnt/storage1/rawdata/ctDNA/list.csv")
  val list2 = INPUT(path = "/mnt/storage2/rawdata/ctDNA/CHICseq2_2/list.csv")
  val list3 = INPUT(path = "/mnt/storage2/rawdata/ctDNA/CHICseq3/list.csv")
  val list4 = INPUT(path = "/mnt/storage2/rawdata/ctDNA/CHICseq4/list.csv")
  val list5 = INPUT(path = "/mnt/storage2/rawdata/ctDNA/CHICseq5/list.csv")
  val list6 = INPUT(path = "/mnt/storage2/rawdata/ctDNA/list_CHICseq6.csv")
  ...
  val fgbio = "/mnt/storage1/tools/fgbio/fgbio-1.1.0.jar"
  ...
  val list123 = CSVDplyr(csv1 = list1,
      csv2 = list2,
      csv3 = list3,
  ...
  val list456 = CSVDplyr(csv1 = list4,
      csv2 = list5,
      csv3 = list6,
      function1 = "bind_rows(csv2)",
      function2 = "bind_rows(csv3)")

  val list = CSVDplyr(csv1 = list123,
      csv2 = list456,
      function1 = "bind_rows(csv2)")
  ...
  val consensusedFilter2 = NamedMap[BashEvaluate]("consensusedFilter2")
  val consensusedFastq2 = NamedMap[BashEvaluate]("consensusedFastq2")
  val consensusAligned2 = NamedMap[BashEvaluate]("consensusAligned2")
  val consensusSorted2 = NamedMap[BashEvaluate]("consensusSorted2")
  val bamOut = NamedMap[Any]("bamOut")
  val bamCorrectedOut = NamedMap[Any]("bamCorrectedOut")
  val bamCorrectedOut2 = NamedMap[Any]("bamCorrectedOut2")
  ...
  val alignmentSummary2 = NamedMap[BashEvaluate]("alignmentSummary2")
  val alignmentSummaryCombined2 = NamedMap[BashEvaluate]("alignmentSummaryCombined2")
  val alignmentSummaryOut2 = NamedMap[Any]("alignmentSummaryOut2")
  ...
  val bySampleCSV2 = CSVDplyr(csv1 = bySampleCSV,
           function1 = """select(everything())""")

  for ( rowMap <- iterCSV(bySampleCSV2) ) {
    ...
    consensusedFilter2(Sample) = BashEvaluate(var1 = consensused(Sample).out1,
        var2 = reference,
        param1 = fgbio,
        script = """
                 java -jar @param1@ FilterConsensusReads \
                   -i @var1@ -o @out1@ -r @var2@ -M 2 -N 15 -q 20
                 """)
    consensusedFilter2(Sample)._filename("out1", Sample + ".bam")
    consensusedFilter2(Sample)._keep = false
    ...
    consensusedFastq2(Sample) = BashEvaluate(var1 = consensusedFilter2(Sample).out1,
        script = s"""
                 java -jar $picard SamToFastq I=@var1@ F=@out1@ F2=@out2@
                 """)
    consensusedFastq2(Sample)._filename("out1",Sample + "_R1.fastq")
    consensusedFastq2(Sample)._filename("out2",Sample + "_R2.fastq")
    consensusedFastq2(Sample)._keep = false
    ...
    consensusAligned2(Sample) = BashEvaluate(var1 = consensusedFastq2(Sample).out1,
        var2 = consensusedFastq2(Sample).out2,
        var3 = reference,
        param1 = Sample,
        script = s"""
                 $bwa mem -M -t 4 -R "@RG\\tID:ID1\\tPL:ILLUMINA\\tLB:Library1\\tSM:@param1@" @var3@ @var1@ @var2@ > @out1@
                 """)
    consensusAligned2(Sample)._filename("out1",Sample + "_ConsensusAligned.bam")
    consensusAligned2(Sample)._keep = false
    ...
    consensusSorted2(Sample) = BashEvaluate(var1 = consensusAligned2(Sample).out1,
        var2   = reference,
        param1 = picard,
        script = """java -Xmx4g -jar @param1@ SortSam VALIDATION_STRINGENCY=SILENT SORT_ORDER=coordinate \
               CREATE_INDEX=true CREATE_MD5_FILE=true INPUT=@var1@ OUTPUT=@out1@ TMP_DIR=$( gettempdir )
               """)
    consensusSorted2(Sample)._filename("out1",Sample + "_CorrectedSorted.bam")
    consensusSorted2(Sample)._keep = false
    ...
    consensusRecal2(Sample) = BashEvaluate(var1 = consensusSorted2(Sample).out1,
         var2 = reference,
         var3 = dbsnp,
         var4 = indelsGold,
         param1 = gatk,
         ...
         script = """
                  ...
                  --bqsr-recal-file @out1@ -O @out2@
                  """)
    consensusRecal2(Sample)._filename("out1","recalibration.table")
    consensusRecal2(Sample)._filename("out2", Sample + "_consensusRecal.bam")
...
*/
   ...
   alignmentSummary2(Sample) = BashEvaluate(var1 = consensusRecal2(Sample).out2,
       var2 = reference,
       var3 = targetsIL.out1,
       param1 = picard,
       script = """
    ...
    alignmentSummaryCombined2(Sample) = BashEvaluate(var1 = alignmentSummary2(Sample).out1,
      var2 = alignmentSummary2(Sample).out2,
      script = """
            cat @var1@ | grep 'CATEGORY\|^PAIR' > @out1@
            cat @var2@ | grep 'BAIT_SET\|^targets' > @out2@
            paste @out1@ @out2@ > @out3@
   ...
   alignmentSummaryOut2(Sample) = alignmentSummaryCombined2(Sample).out3
   }
 
  val stats = CSVListJoin(in = alignmentSummaryOut,
       fileCol = "Sample")
  
  val stats2 = CSVListJoin(in = alignmentSummaryOut2,
       fileCol = "Sample")
  ...
  val bamCorrectedOutCSV2 = Array2CSV(in = bamCorrectedOut2)

...
object ctdna{
  ...
  val list = INPUT(path = "/mnt/storage1/work/amjad/ctdna/result_ctdnaAlignment/bamOutMixed/out.csv")
  ...
  val gatk = "/mnt/storage1/tools/gatk/gatk-4.1.1.0/gatk-package-4.1.1.0-local.jar"
  ...
  for ( rowMap <- iterCSV(listNormals) ) { 
    ...
    ponNormal(key) = BashEvaluate(var1 = normal(key),
       ...
       script = """
           ...
           # @param3@ -jar @param2@ SelectVariants -R @var3@ -V @out1@ -O @out3@ \
           #    --select-type-to-include SNP  --select-type-to-include INDEL              
                """)
...
for ( rowMap <- iterCSV(outBamArrayCSVmerged) ) { 
  ...
  varsFiltered(patient) = BashEvaluate(var1 = vars(patient).out1,
     ...
     script = s"$java8 -jar $gatk FilterMutectCalls -R @var3@ -V @var1@  -O @out1@ --stats @var1@.stats --filtering-stats @out2@ --max-events-in-region 15 --min-median-read-position 15 --log10-somatic-indel-prior -9.0 -L @var4@ " +
...
// -ob-priors @var2@
  ...
  varsAnnotCSVFixed(patient) = CSVDplyr(csv1 = varsAnnotCSV(patient).out1,
     ...
     function7 = """select(1:26, starts_with("FFPE"), starts_with("ctDNA_0"), starts_with("ctDNA_1"), starts_with("ctDNA_2"), starts_with("WB"))""")
}

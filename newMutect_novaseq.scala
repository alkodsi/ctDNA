#!/usr/bin/env anduril
//$OPT --threads 15
//$OPT -d /mnt/storage2/work/amjad/ctdna/result_newMutect_novaseq/
...
object ctdna{
  ...
  val listRaw = INPUT(path = "/mnt/storage2/work/amjad/ctdna/result_ctdnaAlignmentNovaseq/bamCorrectedOutCSV/out.csv")
  ...
  val list = CSVDplyr(csv1 = listRaw,
       function1 = """bind_rows(data.frame(Key = "WB_CHIC_29", File = "/mnt/storage1/work/amjad/ctdna/result_ctdnaAlignment/consensusRecal_WB_CHIC_29/WB_CHIC_29_consensusRecal.bam"))""")
...
for ( rowMap <- iterCSV(outBamArrayCSVmerged) ) { 
  ...
  varsFiltered(patient) = BashEvaluate(var1 = vars(patient).out1,
     ...
     script = s"$java8 -jar $gatk FilterMutectCalls -R @var3@ -V @var1@  -O @out1@ --stats @var1@.stats --filtering-stats @out2@ --max-events-in-region 15 --min-median-read-position 15 -L @var4@ -ob-priors @var2@ " +
